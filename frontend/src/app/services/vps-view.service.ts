...
export class VpsViewService {
  ...
  load(host: string, period?: PeriodOption) {
    ...
    this.http.get<VpsViewData>(url).subscribe({
      ...
      error: () => {
        setTimeout(() => {
          this._data.set(this.demo(host, p));
  ...
  private demo(host: string, period: PeriodOption): VpsViewData {
    return {
      host,
      timeSeries: {
        cpu: this.series(period, 'cpu'),
        memory: this.series(period, 'memory'),
        network: this.series(period, 'network'),
        disk: this.series(period, 'disk')
      }
    };
  }
